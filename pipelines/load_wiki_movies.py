"""
dlt pipeline: Download Wikipedia Movie Plots dataset from Kaggle and load into DuckDB.
...
def load_wiki_movies(target: str):
    ...
    print(df.drop(columns=["Plot"]).head())

    destination, dest_label = get_destination(target)

    pipeline = dlt.pipeline(
        pipeline_name="wiki_movies_pipeline",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Load Wikipedia Movie Plots dataset into DuckDB")
    parser.add_argument(
        "--target",
